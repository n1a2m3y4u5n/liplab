...
### 숙달(mastery) 판정 기준 — `backend/main.py`의 `_STAGE_RULES`
...
- 새 잠금/숙달 규칙을 바꿀 때는 (1)`main.py`의 status 계산, (2)`StageGate`, (3)대시보드 버튼 가드
  세 지점의 정합성을 함께 확인한다.
- 숙달 임계값은 `main.py`의 `_STAGE_RULES`에서만 조정한다.
...
- 프론트 `AvatarVRM.jsx`: 단일 GLB(`/models/realistic_face.glb`)와 ARKit 블렌드셰이프를 사용한다.
- `LipSyncPlayer3D.jsx`: 프레임 스테핑, 속도·프레임 이동·리플레이·자막 동기화·시점/거리 조절을 제공한다.

### 적용된 3D 개선

| 작업 | 구현 내용 |
|------|-----------|
| 전환 시간 반영 | 프레임별 `transition_ms`와 재생 속도를 `AvatarVRM`에 전달 |
| 이징 + 피크 도달 | ease-in-out 보간을 프레임 길이의 60% 안에 완료 |
| 측면 뷰 | 정면에서 확인하기 어려운 원순음·양순음의 입술 돌출 확인 |
| 거리 조절 | 0.45m 클로즈업부터 약 1m 대화 거리까지 카메라 보간 |
| 아이들 모션 | 눈 깜빡임, 미세 머리 움직임, 호흡을 입모양과 독립적으로 적용 |
| 혀 모프 | 혀 전용 모프를 혀 메시에만 적용해 얼굴 왜곡 방지 |

### 후속 개선 후보

- 선행 동시조음: 원순음 등에서 다음 viseme을 미리 블렌딩.
- 웹캠 미러 모드: 아바타 옆에 사용자 입을 표시하고 목표 viseme과 비교.
- 최소대립쌍 A/B 아바타: 단어 두 개의 입모양을 동시에 비교.
- 약점 기반 적응 템포: 취약 viseme 프레임 자동 감속.
- 실제 화자 영상 및 TTS 오디오 동기화.
