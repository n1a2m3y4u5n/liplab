...
// 연습 인사이트(DashboardInsights)의 추천 모드 → 새 정보구조에서 그 활동을 시작하는 실제 화면.
// 예전 대시보드의 모드 선택기는 없어졌으므로, 모드를 고르는 대신 해당 학습 화면으로 보낸다.
// 주관식·4지선다·대화는 모두 문장 학습 화면에서 상황을 고른 뒤 시작한다(config/tracks.js와 동일).
const INSIGHT_MODE_ROUTES = {
  study: '/learn/lipreading/lesson/viseme',
  test: '/learn/lipreading/lesson/sentence',
  'test-multiple': '/learn/lipreading/lesson/sentence',
  conversation: '/learn/lipreading/lesson/sentence',
...
export default function Dashboard() {
  ...
  const openInsightMode = (mode) => navigate(INSIGHT_MODE_ROUTES[mode] || '/learn/lipreading')
  ...
  return (
    <div className="min-h-0 bg-white lg:h-full lg:overflow-hidden">
      ...
      <main className="mx-auto max-w-[1440px] px-4 py-4 sm:px-6 sm:py-5 lg:h-full">
        ...
        {/* 연습 인사이트 — 추천 다음 단계 + 이번 주 학습 흐름. 첫 화면(추천·프로필·히어로) 구성을 건드리지 않도록 그 아래에 둔다.
            레벨·XP는 LearnerProfileCard·TopBar가 이미 보여주고 모드 선택기는 새 구조에 없으므로 둘 다 끈다. */}
        <section id="insights" aria-label="학습 인사이트" className="mt-6">
          <DashboardInsights
            user={user}
            statistics={statistics}
            calendarData={calendarData}
            onSelectMode={openInsightMode}
            onOpenAnalysis={() => navigate('/analysis/overview')}
