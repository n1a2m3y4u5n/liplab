...
function WordQuiz({ data }) {
  ...
  const [correctCount, setCorrectCount] = useState(0)  // 세션 정답 수 → 완료 뷰 XP
  ...
  // 세션 숙달 → 전용 완료 뷰(93:12)로 교체. XP = 세션 정답 수 × 10.
  if (stat.mastered) {
    return (
      <LessonComplete mastery={stat.mastery} xp={correctCount * 10} elapsedSec={elapsedSec}
        onNext={() => navigate('/practice')} onHome={() => navigate('/learn/path')} />
    )
  }
