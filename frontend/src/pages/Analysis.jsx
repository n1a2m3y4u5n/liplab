          ...
          {d.stages?.map((s) => (
            <div key={s.stage}>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-gray-500">{Math.round(s.progress_percent || 0)}% <span className="text-xs text-gray-400">({s.attempts}회 · 성공률 {s.mastery_score}%)</span>{s.status === 'mastered' && ' ✓'}</span>
              </div>
              <div className="bg-gray-100 rounded-full h-2.5 overflow-hidden">
                <div className={`h-full rounded-full ${s.status === 'mastered' ? 'bg-green-500' : 'bg-rose-400'}`} style={{ width: `${Math.min(s.progress_percent || 0, 100)}%` }} />
              </div>
            </div>
          ))}
        </div>
