  ...
  return (
    <AppShell active="analysis" title="분석" description="얼마나 늘었는지 기록으로 확인해보세요.">
      ...
      <Modal open={modal === 'history'} onClose={() => setModal(null)} title="회차 히스토리"
        subtitle={historySubtitle(cal)} maxW="max-w-2xl">
        <HistoryList cal={cal} />
      </Modal>
...
function historySubtitle(cal) {
  const total = Object.values(cal || {}).filter((c) => c > 0).length
  return `최근 학습 기록 · 총 ${total}일`
}
...
/** 회차 히스토리 — Figma 행 레이아웃(시각·트랙칩·제목·문항수·화살표).
 *  getCalendar는 날짜별 횟수만 제공하므로 트랙칩은 '학습'으로 고정, 제목/점수는 생략한다. */
  ...
  return (
    <div className="flex flex-col">
      {entries.map(([date, count], i) => (
        <div key={date} className={`flex items-center gap-3 py-[15px] ${i ? 'border-t-[1.5px] border-line' : ''}`}>
          <span className="w-[92px] shrink-0 text-[12.5px] text-ink-muted">{date}</span>
          <span className="flex w-[54px] shrink-0 items-center justify-center rounded-[8px] bg-[#efe9fc] py-[5px] text-[11.5px] font-bold text-[#5f3ab8]">학습</span>
          <span className="min-w-0 flex-1 truncate text-[15px] font-bold text-ink">독화 학습</span>
          <span className="w-[56px] shrink-0 text-right text-[12.5px] text-ink-muted">{count}회</span>
