...
import { scoreLevel } from '../lib/scoreTone'
  ...
  return (
    <AppShell active="analysis" title="분석">
      ...
      <Modal open={modal === 'history'} onClose={() => setModal(null)} title="회차 히스토리"
        ...
        <HistoryList rows={history} onGo={(to) => { setModal(null); navigate(to) }} />
      </Modal>
 ...
 * 행을 누르면 서버가 준 route(해당 학습 화면)로 이동한다(오답 복습 행은 /review/mistakes).
 */
function HistoryList({ rows, onGo }) {
  ...
  return (
    <div className="flex flex-col">
      {rows.map((r, i) => {
        ...
        const to = isReview(r) ? '/review/mistakes' : r.route
        return (
          <button key={`${r.date}-${r.kind}-${r.topic ?? i}`} type="button" onClick={() => onGo(to)}
