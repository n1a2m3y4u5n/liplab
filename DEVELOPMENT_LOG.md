...
### D.6 OLKAVS 전처리 파이프라인 — 실제 데이터 규모 대응

**배경.** OLKAVS 실데이터(20TB급) 확보를 앞두고 D.5의 전처리 스크립트를 점검한 결과,
샘플 몇 개로는 드러나지 않던 문제 세 가지가 있었다. 20TB를 클라우드에 그대로 올리면
저장 비용만 월 $1,000 규모(RunPod 네트워크 스토리지 1TB 초과 구간 $0.05/GB·월 × 20,000GB)라
계획서의 RunPod 예산 전체(US$737)를 한 달에 넘긴다. 따라서 **로컬에서 오디오·입술 크롭만
남겨 수십 GB로 줄인 뒤 올리는 것**이 유일하게 성립하는 경로이고, 그 전처리가 실제 규모에서
돌아가야 한다.

**문제 ①: 파일 탐색이 O(n²).** `--dir` 경로에서 라벨 JSON **하나마다** 전체 폴더를 다시
`os.walk` 해 mp4를 찾고 있었다. 측정 결과 영상 수가 8배(500→4,000)일 때 시간이 82배로
늘어(0.10초→8.18초) 제곱 증가를 확인했다. 실제 데이터셋 규모에서는 전처리를 시작조차
못 한다. → `index_media()`로 **한 번만 순회**해 파일명→경로 인덱스를 만들도록 교체.
같은 측정에서 4,000개 기준 1,202배 빨라졌다(tmpfs 기준이라 낙관적 — 외장 HDD에서는
순회마다 디스크 I/O가 붙어 격차가 더 커진다).

**문제 ②: 직렬 처리.** ffmpeg를 하나씩 순차 실행했다. → `ThreadPoolExecutor`로 병렬화
(ffmpeg는 별도 프로세스라 대기 중 GIL을 놓으므로 스레드로 충분). `--jobs` 기본값은 CPU 코어 수.

**문제 ③: 오디오를 버림(`-an`).** 축 D(립리딩)용으로 작성돼 영상만 남겼는데, 계획서가
"최대 병목"으로 지목한 축 A(백본·농인 발화 합성)와 축 B(D-GOP)는 **오디오가 필요**하다.
→ `--mode audio|video|both` 추가. 오디오는 영상 디코딩을 건너뛰어(`-vn`) 훨씬 빠르므로
축 A·B를 먼저 착수할 수 있다. 문장별로 원본 영상을 다시 여는 대신 **영상당 전체 오디오를
한 번 뽑아 두고 거기서 잘라낸다**(디코딩이 문장 수만큼 반복되지 않게).

**추가.** 며칠 걸리는 작업이라 **중단 후 이어서 돌리기**를 넣었다(이미 만든 산출물·라벨은
건너뛰고, 라벨은 append+flush로 중단 시점까지 보존). 깨진 라벨 JSON·ffmpeg 실패는 배치를
멈추지 않고 건너뛰며 마지막에 실패 건수를 보고한다. `--limit`으로 영상 수를 제한해
부분집합만 뽑을 수 있다(계획서가 정한 "소규모 프로토타입" 범위에 맞춤).

**검증.** 순수 로직 테스트 8개 신규(`scripts/test_preprocess_olkavs.py`) — 크롭 박스가 입술
영역을 실제로 덮는지·프레임을 벗어나지 않는지, 인덱싱이 중첩 폴더 경로를 정확히 잡는지,
모드별 작업 생성(오디오는 원본 영상이 아니라 전체 오디오에서 잘리는지), 이미 만든 산출물
건너뛰기, 깨진 메타데이터 내성. **ffmpeg 미설치 환경이라 실제 변환은 검증하지 못했다** —
데이터 도착 후 샘플로 확인이 필요하다.
...
## A-보정. 표시용 점수 보정 (2026-09-09)

A-실행이 남긴 **한계 ②**에 대한 조치다. 변별력은 합격했는데 눈금이 사람 것이 아니었다 —
깨끗한 발화가 9.51/100이라 `speak_curriculum`의 합격선(음소 50 / 단어·문장 65)과 **비교조차
되지 않는다**. 이대로 앱에 연결하면 모든 학습자가 항상 0점대를 본다.

### 원점수는 건드리지 않는다

먼저 정한 원칙이다. 원점수(`naive × confidence`의 구간 평균)를 키우려고 채점식을 손대면,
A-3에서 잰 변별력이 그대로 유지되는지 다시 증명해야 한다. 대신 **표시용 점수를 따로 만들고
원점수는 응답에 함께 실어 보낸다**(`score` / `raw_score`). 채점의 근거와 학습자가 보는 숫자를
분리하는 셈이다.

### 방법 — severity 앵커 + log 보간

기준 발화 집합의 severity별 원점수 대표값을 앵커로 잡고, "이 정도 발음은 몇 점으로 보여야
하는가"에 잇는다.
...
앵커 사이는 **log 공간에서 선형 보간**한다. 원점수가 severity에 따라 대략 기하급수로 줄기
때문이다(9.51 → 2.74 → 1.49 → 0.61 → 0.30 — 구간별 기울기가 log 축에서 33~65점/decade로
비슷하게 모인다). 선형 공간에서 이으면 상위 구간이 뭉개져 잘한 발음끼리 구별이 안 된다.

양끝: 원점수 0을 0점, 최상위 앵커의 기울기를 그대로 연장한 지점(원점수 ≈ 18.9)을 100점으로
둔다 — 정상 발화 대표값보다 **뚜렷하게 좋아야** 만점이 나오게 하기 위함이다. log 변환에는
유사계수(`CALIBRATION_FLOOR = 0.05`)를 더해 원점수 0을 다룬다.

표시 목표(`DISPLAY_TARGETS_BY_SEVERITY`)는 측정값이 아니라 **합격선에서 역산한 제품 결정**이다.
코드에 상수로 박아 두고 근거를 주석에 남겼다 — 나중에 바꿀 때 무엇을 바꾸는지 알고 바꾸도록.

### 이 보정이 하지 않는 것

**단조 증가 변환이다.** 순위를 바꾸지 않으므로 A-3 변별력 지표(순위상관·AUC)는 보정 전후가
같다. 변별력을 사후에 만들어내는 것이 아니라, 이미 있는 변별력을 읽을 수 있는 눈금으로 옮길
뿐이다. 이 성질이 깨지면 채점이 조작이 되므로 테스트로 고정했다
(`test_calibration_preserves_ranking`).

원점수가 severity를 거스르는 구간은 앵커에서 **버린다**. A-3 베이스라인의 3→4 역전(1.15 →
1.25)을 그대로 앵커로 삼으면 보간이 뒤집혀 **더 나쁜 발음에 더 높은 점수**가 간다. 버린
severity는 출력(`dropped_severities`)과 스크립트 경고에 남긴다 — "이 모델은 그 구간을 구별하지
못한다"는 사실을 숨기지 않는다.
...
| `backend/dgop_acoustic.py` | `load_calibration` + `assess_text`가 `score`(보정)·`raw_score` 동시 반환 |
| `scripts/fit_dgop_calibration.py` | 기준 발화로 앵커 재적합 → JSON |

앵커 우선순위는 `DGOP_CALIBRATION`(경로) → `backend/data/dgop_calibration.json` →
`dgop.DEFAULT_CALIBRATION`(A-3 실측 내장값). **파일이 없거나 깨져도 내장값으로 떨어져 채점
자체는 죽지 않는다** — 보정 파일 하나 때문에 발음 채점 전체가 멎는 것은 균형이 맞지 않는다.

재적합 스크립트는 A-3과 **같은 채점 경로·같은 시드**를 쓴다(`eval_dgop_discrimination`을 그대로
import). 앵커를 잰 방식과 합격을 판정한 방식이 어긋나면 두 숫자를 나란히 놓을 수 없다.
다만 대표값은 A-3의 평균이 아니라 **중앙값**을 쓴다 — 평균은 발화 몇 개의 극단값에 끌려가고,
앵커가 흔들리면 같은 발음에 다른 점수가 나온다.

`calibrate=False`로 보정 이전 값을 볼 수 있는 문도 남겼다(변별력 평가처럼 원점수를 봐야 하는 쪽).

### 검증

`test_dgop.py` 10개 / `test_dgop_acoustic.py` 8개 통과. 새로 추가한 것은 앵커 적중, 양끝·범위,
**순위 보존**, 합격선 통과 여부(정상 발화 ≥ 65 / 중증 < 50), 역전 severity 제외, 앵커가 부족한
입력의 예외, 보정 파일 우선순위와 깨진 파일 폴백.

### 남는 한계

앵커가 **합성 저하** 기준이다. "severity 1 = 72점"이 실제 농인 학습자의 체감과 맞는지는
검증되지 않았다 — 한계 ①과 같은 데이터 부재에 걸려 있다. 실제 발화 표본이 생기면 표시
목표부터 다시 본다.

내장 기본값은 A-3 리포트의 **평균**에서 뽑았고, 스크립트는 중앙값으로 다시 잰다. **체크포인트
전용**이므로 모델을 바꾸면 반드시 재적합한다(런북 §6.5를 앱 연결 전 필수 단계로 넣었다).

**변경 파일(A-보정).** `backend/dgop.py`·`dgop_acoustic.py`·`main.py`·`test_dgop.py`·
`test_dgop_acoustic.py`, `scripts/fit_dgop_calibration.py`(신규),
`docs/axis-a-training-plan.md`(§A-4)·`axis-a-runbook.md`(§6.5).
