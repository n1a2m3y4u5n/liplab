...
> **2026-09-14 — 이 절은 로컬에서 이미 통과했다.** `backend/venv`의 파이썬으로 돌린 결과:
> 테스트는 backend 22파일·scripts 3파일·frontend 35케이스 전부 통과. `--smoke`는 **31초에
> 구간 240개**(severity별 48개, `mean_*` NaN 행 0개, **blank 섞인 구간 0개**)를 떴고, 이어서
> `sweep_gop_scorers.py`까지 채점식 9종·부트스트랩 CI 표가 정상 출력됐다. 공개 체크포인트는
> HF 캐시에 있어 다운로드도 없었다. **그래서 Pod에서는 이 절을 건너뛰어도 된다.**
>
> 단 하나 남는다: 로컬은 **음절 vocab 공개 모델**이라 `--mode perturb`가 가드에 걸려 `exit 2`로
> 끝난다(정상 동작 — 함정 §3-5 참고). **자모 체크포인트에서 가드를 통과하는지는 로컬에서
> 확인할 수 없고, E2 첫 발화가 유일한 확인 지점이다.** 그래서 실행 순서(① 백업 → ② E1 → ③ E2)는
> 그대로 둔다.
...
- ~~**blank 집계는 문제가 아니다.**~~ → **2026-09-14 철회(A-6 실측).** `token_spans`가 run으로
  잡는 것은 **정렬 경로의 라벨**이다. 그 라벨에 blank가 없다는 것과 '그 프레임에서 모델이 blank를
  argmax로 고르지 않는다'는 것은 **다른 얘기**다. 강제정렬은 모델이 blank를 선호하는 프레임에도
  목표 토큰을 배정하므로, A-6에서 **E1 구간의 47%(10,785개)**가 blank 우세 프레임을 포함했다.
  구간 평균이 그 프레임을 포함하니 **Cao et al. 2024의 지적은 이 구현에도 유효하다.**
  불변 검사(`nb_frames != frames`)는 '정렬이 깨졌는지'를 재지 못한다 — 경고 문구와
  `test_dump_gop_features`의 단언을 함께 고쳤다.
...
- E2의 오염은 **치환과 탈락**을 모사한다 — `jamo_perturb.RULES`는 종성 탈락·마찰음 파열음화·
  비음화·모음 중앙화 4종이다. 초판에 "치환 오류만"이라고 적은 것은 틀렸다(2026-09-14 정정).
  실제 농인 발화의 운율·타이밍 이상은 재현하지 않는다. A-6에서 **탈락은 구간 평균 기반 점수가
  구조적으로 못 잡는다**는 것이 드러났다(AUC 0.482 = 우연) — 탈락은 '틀린 구간'을 만들지 않고
  구간을 없애기 때문이다.
