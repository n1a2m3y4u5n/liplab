...
>   AI 대화까지 눌러 봄). 학습 화면 3D 플레이어에 보이던 개발용 'Viseme N · ms' 표시를 뺐다(1b80ab2). 남은 개선 여지: 분석 상세
>   (취약 입모양·평균 점수)가 AI 추천 문구(`/api/analysis`, 약 4초)를 기다린 뒤에야 뜬다. 팀원 IA 모듈(`frontend/src/features/`)은 여전히 미연결.
