...
> **2026-09-26: liplab-dev 콜드 스타트 개선·4단계 버튼 수정·UI 변경 내역 적용, 저녁에 배포(사용자 지시, v9). 전체 검토 뒤 결함 수정(배포 안 함).**
> - **배포(19:47, v9).** f53fc31·52c7536·5765707을 올렸다. 정렬기·채점기가 int8 파일에서 각 22초에 올라오고(전에는 76초) 켜진 뒤
>   2분 14초에 모두 준비된다(전에는 약 4분). 첫 채점 2.7초·84.1점, 메모리 최고 2.40GiB. 일시정지는 쉰 지 2.5~4분 뒤 저절로 됐고,
>   두 번 깨운 것 중 한 번은 1.8초에 모델이 그대로 있었으나, 한 번은 fly가 기계를 다른 호스트로 옮기며(이주) 새로 켜졌다(앱 응답
>   73초, 이어서 예열 2분 15초). `DEPLOY.md` 9항.
> - **전체 검토(코드 검토 두 갈래, 테스트, 배포 화면 확인) 뒤 고친 것, 배포 안 함.** 상태 확인 주소 두 곳(`/api/backbone/status`,
>   `/api/avatar/audio2face/status`)이 처음 부를 때 torch를 불러오며 서버를 잠깐 멈출 수 있어 스레드에서 부르게 했다. 단독으로 돌리면
>   실패하던 새 테스트를 고쳤다. fp32와 int8 파일이 함께 있으면 GPU는 fp32로 올린다. 빌드 점검은 켜질 때와 같은 적재 경로(전처리기
>   포함)로 돌리고 적재 경로·torch 판을 기록한다. DEPLOY.md의 체크포인트 복사 명령이 하위 폴더를 만들던 것을 고쳤다. f53fc31이
>   Practice·Conversation 두 파일의 줄바꿈을 CRLF에서 LF로 통째로 바꾼 것도 되돌렸다(실제 변경 13줄, 팀원 브랜치와 병합 충돌 방지).
>   화면 쪽: 스크롤 안내 목록은 필터를 바꾸면 맨 위로 가고, 목록이 빠지면 카드 여백이 돌아온다. 스크롤이 생기면 키보드 초점을 받고,
>   초점 항목이 페이드에 가리지 않는다. 스크롤 연쇄를 막던 설정은 뺐다. 난이도 슬라이더는 어디를 눌러도 가까운 단계로 간다(iOS는
>   원래 끌어야만 움직였다). 여러 명 대화에 난이도를 넘긴다(한 턴 길이, 1단계 4~8자부터 5단계 14~22자까지). 자유 발화 성도 단면은
>   입모양이 자리 그림으로 돌아갈 때 함께 쉼으로 돌아간다. 프론트 테스트 80, 백엔드 259 통과, 빌드 통과.
> - 멈춘 기계를 깨우면 모두 준비되기까지 약 4분이었다(정렬기·채점기 적재 각 76초, 첫 아바타 요청 76초). 원인은 CPU가 아니라
>   루트 파일시스템 읽기(초당 약 17MB)였다. 세 가지를 고쳤다. 일시정지(`auto_stop_machines = "suspend"`, 수동 시험에서 깨운 뒤 첫
>   채점 3.6초·첫 아바타 3.1초, 요금은 멈춤과 같음), 미리 변환한 int8 파일(모델당 355MB, 실행 중 변환과 비트 단위로 같고 D-GOP 점수
>   같음), 예열에서 아바타 백본까지 미리 읽기. 적재 중 `/api/backbone/status`가 서버 전체를 멈추던 결함도 고쳤다. 빌드 폴더 업로드가
>   2.6GB에서 약 0.8GB로 준다(`DEPLOY.md` 9항 "콜드 스타트"). 백엔드 pytest 257 통과.
> - 화면 버튼 전수 점검(데모 계정, 로컬): 4단계 경로 버튼 말고는 눌러도 반응 없는 버튼이 없었다.
> - 계획서 이행률 75%(부분 인정 83%, 9/25 72%·82%). 배포로 B 61→79%, A 77→86%. D는 제품 립리딩이 처음 보는 사람에게
>   우연 수준이라 D-5를 부분으로 낮춰 100→95%. 남은 77개 중 68개가 사용자·사람 몫
>   (`liplab-lab/notes/plan_audit/progress_2026-09-26.md`).
> - UI 변경 내역(`LIPLAB_UI_CHANGELOG.md`, 디자인 쪽 문서) 적용: 연습 탭 다자 대화 제거·수어 보기, 연습 기능 화면 나가기 X,
>   자유 발화 카드 하나(입모양 | 성도 단면), 수어 보기 단순화, 상황별 시나리오 전면 개편(카드 하나·1~5 슬라이더), 스크롤 안내를
>   버튼에서 장식으로(`ScrollHintList`), 404·문제 발생 화면, 퀴즈 공개 상태 높이 보정. 변경 내역과 Figma가 다른 곳과 남은 결정은
>   `LIPLAB_UI_HANDOFF.md` §6-8. 9/26 저녁 배포.
> - 노션 'CNSAi 고도화 관리'(재단이 매주 수요일 확인)를 채웠다: 진행 상황(세부) 축별 페이지 A~K(목표·결과물·실측·남은 것·진행
>   과정, 체크박스는 A 자원·구현, E 자원, K 자원·구현을 실제에 맞춤)와 활동 기록 '주간 활동(9월 5주)'(도움 요청: 파일럿 기관·
>   언어재활사 자문·윤리 심의 경로). 노션 워크스페이스가 무료 블록 한도를 넘어 9/29 15:54(KST)까지만 새 내용을 쓸 수 있다.
> - 남은 사용자 몫: 검토 수정분 배포("배포해"), UI 결정 네 가지(엔드리스 '문맥으로 풀기' 카드, 자유 발화 A4 카드, 여러 명 대화
>   라우트 삭제, 상황별 시나리오 X의 목적지, `LIPLAB_UI_HANDOFF.md` §6-8). 그 밖은 9/25 목록과 같다.

> **2026-09-25 저녁: liplab-dev 배포(사용자 지시).** https://liplab-dev.fly.dev 에 통합 브랜치(14a232f)를 올렸다. 자체 채점 모델
> int8(4GB·CPU 2개), 모델 세 개 적재 확인, 실제 채점 D-GOP 경로(맞는 문장 84.1 대 다른 문장 29.5, 2.4~2.8초), 새 앱 아이콘과
> 랜딩 개발자 정보(K-AI 공모전 B트랙 중고등부 대상 수상작, 만든 사람들) 반영. 볼륨 암호화 켜짐 확인. 전시앱 liplab.fly.dev와
> KT 비교 데모(liplab-speaker-demo)는 그대로. 켜진 뒤 첫 채점은 모델 적재(약 75초씩)를 기다린다(DEPLOY.md 9항).
> 같은 날 저녁: 새 화자 받기 도구가 참여자 번호 E 계열을 버리던 결함 수정, 립리딩 입력을 화면 주사율과 무관하게.
> P3(립리딩 채점만 바꾸기) 새 화자 26명: 21.8% → 25.9%, +4.1점 [2.1, 6.3]으로 관문(+5) 미달, 제품 그대로. 4지 우연 25%라 제품 단어
> 독화는 처음 보는 사람에게 아직 우연 수준(`docs/d-bench-v2.md` 8절). GPU 잔액 $8.71.

> **2026-09-25 오후: GPU 세 가지(사전등록 두 개 + 보조 측정), liplab-dev를 int8로 4GB에 맞춤(설정만, 배포 안 함).**
> - **int8 채점 모델 채택.** 정렬기·채점기 선형층 가중치를 int8로(`backend/quant_int8.py`, `BACKBONE_QUANT=int8`). 사전등록
>   (`liplab-lab/notes/int8_prereg_2026-09-25.md`) 다섯 조건 통과: 재검 관문 재현 +3.5%p [2.2, 5.0], 538 표시 점수 차 평균 0.20점,
>   608 실발화 0.36점, CPU·GPU 경로 차 0.1점, 호스팅 점검 최대 3.15GB(채점 뒤 2.7GB, fp32 4.3GB). `fly.dev.toml`을 4GB·CPU 2개로
>   되돌렸다. 처음 점검에서 변환 뒤 체크포인트 파일 매핑이 남아 6.2GB로 재진 것을 고쳤다(`docs/scorer-selftrain.md` 7절).
> - **립리딩 P2 모델 + P1 해독, 새 화자 12명 확인: FAIL, 제품 그대로.** 제품에서 할 수 있는 입력 다듬기(트리머)를 넣은 제안이
>   23.9% → 28.3%, +4.4점 [0.9, 8.4](관문 +5점 미달). 오른 몫은 대부분 채점 방식에서 오고 새 모델만으로는 −1.8점
>   (`docs/d-bench-v2.md` 7절). 사후 관찰로 "채점만 CTC 로그우도로"가 +6.5점(개발 화자 +3.0)이라, 관문을 먼저 적고
>   (`liplab-lab/notes/p3_score_prereg_2026-09-25.md`) 새 화자를 받는 중(맥, 한국 IP).
> - **음소 단위 교체 검출(보조).** 한두 음절 ㅁ↔ㅂ·ㄴ↔ㄷ 바꿈은 문장 점수로 AUC 0.68이지만, 우리 모델 초성 점수로는 깨끗한
>   발화에서 AUC 0.99, 가장 낮은 초성이 바꾼 자리인 비율 93%. 저하가 커지면 0.73(중)·0.60(심)으로 떨어지고 고정 문턱은 못 쓴다.
>   음소 피드백은 상대 순위로 보이는 쪽을 제안(`docs/scorer-selftrain.md` 6절).
> - 파드 CPU가 nproc 96이어도 cgroup 한도 10.2코어였다(`run7.sh`가 몰랐음 → `run7b`·`run7c`로 다시 조율). GPU 약 $1.07, 잔액 $8.94.
> - 남은 사용자 몫: "배포해"(liplab-dev, 이제 4GB 기계), 그 밖은 아침 목록과 같다.

> **2026-09-25 아침: 자체 학습 채점 모델 채택(liplab-dev 설정만, 배포 안 함)과 608 실발화 측정.**
> - **자체 정렬기·채점기.** 팀원 체크포인트(옛 채점식 기준) 대신 같은 레시피(`scripts/train_jamo_ctc.py`·`train_aligner.py`)로
>   Zeroth에서 다시 학습(RunPod A100 약 3시간). 사전등록 관문(538 합성 저하 강도 순서 +2%p 이상, 화자 부트스트랩 CI 하한 > 0):
>   1차 10화자 +2.7%p [0.0, 5.7] FAIL, 1차 뒤 등록한 독립 재검 20화자 +3.8%p [2.3, 5.5] **통과**. 608 실발화 분리도 0.892 → 0.933.
>   다른 문장은 두 모델 모두 합격선을 못 넘고, 한두 음절 자음 바꿈은 둘 다 문장 점수로 못 거른다. 전부 `docs/scorer-selftrain.md`.
> - **liplab-dev를 자체 모델로.** `fly.dev.toml` `DGOP_MODEL=ours`, 체크포인트는 `backend/models/dgop_ours/`(git 제외, 빌드 폴더에
>   APFS 복제로 둠, 없으면 빌드가 멈춤), 앵커 `backend/data/dgop_calibration_ours.json`(538 중앙값 → 90·72·58·40), 기계 8GB·CPU 4개
>   (점검 최대 5.0GB, 문장당 1.6~2.0초). 되돌리기는 `DEPLOY.md` 9항. 새 가상환경 점검에서 SQLAlchemy 2.1이 greenlet을 빼 서버가
>   안 뜨는 결함을 고쳤다(ba3f8a7).
> - **608 확보(감음신경성 27명 669문장).** CER 0.66(정상 0.19), D-GOP 38(정상 80)로 합성 경~중 사이, 그림 7에 608 추가. 지금
>   kresnik 앵커로 608은 중앙값 66점이라 앵커 유지(A-7). kNN-VC 음성변환은 규칙 합성보다 실제 발화를 덜 닮아 부정 결과(A-2·A-10).
> - 계획서 이행률 72%(부분 인정 82%), HF 토큰 대기 11개가 모두 풀림(`liplab-lab/notes/plan_audit/progress_2026-09-25.md`).
>   GPU 원장·집행대장 갱신(사용 $64.99, 잔액 $10.01과 맞음). 파드 없음. 백엔드 pytest 249 통과.
> - **랜딩 페이지(Figma 02. Landing 9:12) 구현.** 비로그인 `/`에서 보이고, 시작하기 → `/signup`(회원가입 모드), 계정이 이미
>   있습니다 → `/login`. 1440 폭에서 제목·부제·버튼 좌표와 전체 높이(3952px)가 Figma와 같다. Figma는 데스크톱뿐이라 lg 미만은
>   세로로 쌓았다(휴대폰·태블릿·1024 확인, 가로 넘침 없음). 마스코트 SVG 16개는 Figma에서 받아 `public/ui/lp-*`에 두었다.
> - **Figma 전수 대조(5페이지·프레임 72개, 글자 295개).** 사용법 가이드 02~11이 Figma와 다른 자체 문구·아이콘 카드로 남아 있어
>   11개 탭 전부 Figma대로 바꿨다(화면 축소본 11장은 Figma에서 PNG 2배로 내보냄, 05 독화 요령, 11 개발자 소개). 통합 병합에서
>   빠진 다자 대화 부제를 되살리고 엔드리스 소개문을 Figma 원문으로. 남은 차이와 사유는 핸드오프 §6-6, 기록은
>   `liplab-lab/notes/figma_audit/recheck_2026-09-25.md`. 다자 대화 구조(Figma 화자 이름 카드 대 축 H 연구 기능)는 결정 필요.
> - 남은 사용자 몫: "배포해"(liplab-dev, 8GB 기계 비용 확인), 파일럿 윤리·협력기관, 보관 기간·보호책임자, 공개 채널, CC5 얼굴,
>   SpeechSuper 구매 여부, 다자 대화 구조.

> **2026-09-24 밤 — 배포 전 점검과 Figma 재점검 후속(코드만, 배포 안 함).**
> - **배포 전 파드 점검 통과.** liplab-dev 추론 설정과 같은 의존성(torch CPU 2스레드)에서 최대 메모리 3.1GB, D-GOP 채점 문장당
>   1.4~1.7초, 맞는 문장 86~95점·다른 문장 0~45점, 입모양 점수는 따로, 데모 계정만 전 단계 열림(`DEPLOY.md` 9항). 이 점검에서
>   음성구동 아바타가 torchaudio·librosa 없이 꺼지던 결함을 고쳤다(b7973dc). 파드는 종료, 비용 약 $0.06.
> - **Figma 재점검 90항목**(해결 59, 의도된 차이 20, 부분 10, 미해결 1, `liplab-lab/notes/figma_audit/recheck_*.json`)의
>   부분·미해결 11개를 모두 반영했다: 건너뛰기가 실제로 단계를 열고 경로에 머묾(발화는 새 `POST /api/speak/skip`, 6c5596d),
>   레슨 화면 이모지 제거(5686e49), 사용자명 2~50자와 입력칸 안내 색·여백(86c815d), 셸 안 로딩을 기본 로딩으로 통일하고 앱 진입
>   스플래시 제거(5d26186), 분석 '총 N회'를 회차 하나로·복습 목록 '3일 전 · 2회 틀렸어요'·모바일 과제 배지 카드 숨김·수어
>   나란히 보기 정리·남은 hex를 토큰으로(52ebd8b).
> - 확인: 백엔드 pytest 248 통과, 프론트 node --test 72 통과, vite build 통과. 브라우저에서 계산 색이 바꾸기 전과 같은지 확인.
> - **전 경로 점검:** 라우트 43개를 데모 계정으로 모두 띄워 오류 화면이 없고, 새 탭에서 콘솔 오류도 없었다(시나리오 → 문장 연습 채점 →
>   AI 대화까지 눌러 봄). 학습 화면 3D 플레이어에 보이던 개발용 'Viseme N · ms' 표시를 뺐다(1b80ab2). 분석 상세(취약 입모양·평균 점수)는
>   화면에 나오지도 않는 AI 추천 문구(`/api/analysis`, 약 4초)를 기다리던 호출을 빼 0.5~0.8초에 뜬다. `/api/analysis`는 이제 프론트가 부르지 않는다
>   (추천 문구를 다시 보이려면 Figma에 없는 카드가 필요). 팀원 IA 모듈(`frontend/src/features/`)은 여전히 미연결.
>   이어서 분석 상세·학습 효과 리포트를 앱 셸로 옮기고 색을 토큰으로(8f8201e), 쓰지 않는 LLM 추천(`generate_analysis_recommendation`)·
>   링크 없는 `/guide` 페이지 삭제(같은 커밋), README·QUICKSTART를 지금 앱 기준으로 갱신(5166aa2).
>   **D 제품 립리딩 재학습(P2) 관문 FAIL**(사전등록 `liplab-lab/notes/p2_prereg_2026-09-24.md`): 30화자 중 18명으로 다시 학습한 모델이
>   평가 10명에서 지금 제품 해독 기준 +0.9점 [−1.0, 3.3](시드 평균 0.0) → 제품 모델 그대로. 참고로 후보 로그우도 채점과 함께 쓰면
>   세 시드 모두 +5.1~5.8점이지만 등록 밖 조합이라 새 화자로 재확인할 일(`docs/d-bench-v2.md` 6절). RTX A6000 37분, 오늘 파드 비용 약 $0.44.
> - 남은 사용자 몫: `fly auth login` 뒤 "배포해", HF 토큰(팀원 모델 재측정), 608 재시도 여부, 파일럿 윤리·협력기관, CC5 얼굴.
...
> - **K-4 화자 영상 측정(부정 결과).** K 비음 확률이 울림 음절과 같은 입모양 파열음 음절을 가르지 못함(음절 AUC 0.486
>   [0.466, 0.508], 200클립 30명). 같은 창으로 모음 개방 0.752·양순 폐쇄 0.718이라 정렬 문제 아님. **결정 필요:** 웹캠 울림 기호
>   (K→J)를 끌지 '실험'으로 표시할지(`docs/cue-video-demo.md`).
> - **4.2-6 GPU 원장.** RunPod billing API로 파드 24대 사용액을 받아 축별로 나눔(`liplab-lab/notes/gpu-ledger.md`, 집행대장
>   GPU원장 시트). 사용 $55.66(잔액 $19.34와 맞음), H100 5.2시간. 파드 1대 $1.72는 다른 과제 사용(**결정 필요:** 자부담 처리).
> - fly.io 볼륨 암호화 확인(4.9-①)은 이 맥이 fly에 로그인되어 있지 않아 못 했다(배포할 때 `fly volumes list`로 확인).
> - **초안 두 개(팀 결정용).** 1구간 점검점(4.4-7, `docs/checkpoint-2026-09.md`: 축별 목표 수준 유지·하향 권고와 보고서에 적을
>   설계 편차 12개), 콘텐츠 월간 생성·검수 루틴(G-10, `docs/content-routine.md`).
> - 확인: 프론트 node --test 69 통과, vite build 통과(백엔드 코드 변경 없음).
