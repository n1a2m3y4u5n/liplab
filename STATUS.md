...
> **2026-09-26 밤: 브랜치 전체 검토(백엔드 보안·학습 로직, 프론트 흐름·미디어, 배포·문서 다섯 갈래 + 배포 서버 전 경로) 뒤 결함 수정, 배포 안 함.**
> - **급한 것 두 가지(dev에 살아 있음).** (1) 로그인 화면이 로그인·회원가입 뒤에도 `/api/seed-demo`를 불러(9/19 ac285d2부터) 실제 계정에
>   가짜 기록(약 130문장, XP 6800, 1·2단계 숙달)을 넣고, 표시가 없는 예전 계정은 연습 기록을 지웠다. dev에는 실제 계정이 1개(9/25 가입)
>   있고 가입 직후 채워져 지워진 기록은 없다. 이제 데모 입장에서만 부르고 서버·채우기 함수 둘 다 데모 계정만 받는다. 전시앱은 7/15
>   판이라 해당 없다. (2) 데모 계정 2단계 행이 두 벌(9/14)이라 dev에서 단어 답 제출이 500이었다. 켜질 때 중복을 합치고 고유
>   인덱스를 건다(단계 3표·약점 입모양·복습 항목), 조회도 여러 행에 견딘다. 데모 계정은 분석 초기화를 못 하게 막았다.
> - **학습 로직.** 숙달은 한번 되면 유지, 뒤 단계는 직전 단계가 열려 있어야 열림, 잠긴 단계의 답은 숙달에 넣지 않음(문맥 추론으로
>   2·3단계가 잠긴 채 4단계가 열리던 것), 정답을 본 뒤 제출은 `practice_only`로 점수만(다시 풀기 네 번이면 3단계 숙달되던 것).
>   오늘 날짜는 한국 시간(서버 UTC라 오전 9시 전 학습이 전날로 잡힘), 복습일 전 성공은 간격을 늘리지 않음, '복습왕' 배지 조건,
>   시나리오 4·5단계 글자 수 제한(늘 쉬운 대체 문장으로 떨어지던 것), 자유 입력 상황의 대화 대체 문구(0으로 나눠 500),
>   받침 없는 문장의 받침 팁, 분해형 한글 입력, AI 단어 규칙 검사, LLM 호출 30초·재시도 1번.
> - **보안·자원.** 채점 입력 300자·본문 크기 상한(긴 입력 하나로 서버가 멈추던 것), 추론 한 번에 하나·음성 앞 30초, 비밀번호 72바이트
>   초과는 거절(500이던 것), 가입 이메일 소문자 저장·대소문자 중복 금지(운영자 권한 우회 방지), XP·점수 값 범위 고정,
>   docker-compose 기본 서명키 제거. Dockerfile은 Whisper보다 먼저 `HF_HOME`을 정하고 torch 2.14.0·transformers 5.17.0 고정
>   (다음 빌드는 ML 층을 새로 만든다).
> - **화면.** 복습 탭 '오답 복습하기'가 입모양·단어 복습(`/review/scheduled`)으로(예전에는 문장 오답만 보여 풀 길이 없었다), 북마크
>   다시 연습이 단계 잠금에 막히지 않게, 입모양을 못 받으면 대화·문장 연습이 멈추지 않게, 불러오기 실패 화면에 다시 시도·나가기,
>   첫 화면은 서버의 배치 여부로 온보딩 결정(공용 기기 대응), 로그아웃 때 계정별 저장값 정리, 모달 포커스 가두기, 휴대폰 회차 기록
>   제목, 프로필 통계, 웹캠 거울이 두 번째 시도부터 검게 되던 것, 음성 아바타가 화면을 떠난 뒤 녹음을 올리고 다른 화면에서
>   재생하던 것, 늦게 온 채점 결과가 다음 시도를 덮던 것, 입모양 점수 포화, 카메라·마이크 오류 문구, 3D 로드 실패 재시도.
> - 문서: CLAUDE.md 숙달 기준표(7/13 뒤 코드와 달랐다)와 3D 모션 A·B·D·F(7/14 병합에서 코드가 빠졌는데 완료로 적혀 있었다),
>   DEPLOY.md 주의(전시앱 옛 절차, 모델 폴더가 이미지에 들어가는 문제, 운영자 이메일 등록 순서, 서버 검수 화면 제약).
> - 테스트 백엔드 261(새 회귀 테스트 `test_review_fixes_0926.py`), 프론트 80, 빌드 통과. 로컬 브라우저로 복습 경로, 다시 풀기
>   `practice_only`, 첫 화면 온보딩, 모달 포커스, 휴대폰 회차 기록을 확인했다.
> - 남은 사용자 몫: 검토 수정분 배포("배포해"), 데이터·모델 공개 범위 결정(세부는 세션 보고), dev 실제 계정 1개의 가짜 기록 정리
>   여부, 객관식 답 채점 방식(비슷한 오답이 통과로 잡힘, 서버 필드 필요), 답에 없는 말을 덧붙여도 감점이 없는 채점식, 파일럿
>   사전·사후 검사를 서버 문항으로 채점할지, 공용 데모 계정 구조, 외부 CDN(MediaPipe·ONNX) 자체 호스팅.
